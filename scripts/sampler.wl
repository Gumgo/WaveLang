...
module real sampler_sin(in real frequency) {
	return sampler_loop("__native_sin", 0, false, frequency_to_speed(440, frequency), 0);
}

module real sampler_sawtooth(in real frequency) {
	return sampler_loop("__native_sawtooth", 0, false, frequency_to_speed(440, frequency), 0);
}

module real sampler_triangle(in real frequency) {
	return sampler_loop("__native_triangle", 0, false, frequency_to_speed(440, frequency), 0);
}

module real sampler_pulse(in real frequency, in real phase) {
	real a := sampler_loop("__native_sawtooth", 0, false, frequency_to_speed(440, frequency), 0);
	real b := sampler_loop("__native_sawtooth", 0, false, frequency_to_speed(440, frequency), phase);
...
module real sampler_ext(in string sample, in real channel, in bool loop, in bool bidi, in real speed) {
	...
	return select(loop_const,
		sampler_loop(sample, channel, bidi, speed, 0),
		sampler(sample, channel, speed));
}