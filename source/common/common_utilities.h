...
constexpr t_value swap_byte_order(t_value value) {
	t_value result;
...
template<typename t_value>
constexpr t_value clamp(t_value value, t_value lower, t_value upper) {
	return std::min(std::max(value, lower), upper);
}

#if IS_TRUE(ENDIANNESS_LITTLE)
template<typename t_value>
constexpr t_value native_to_little_endian(t_value value) {
	return value;
}

template<typename t_value>
constexpr t_value native_to_big_endian(t_value value) {
	return swap_byte_order(value);
}

template<typename t_value>
constexpr t_value little_to_native_endian(t_value value) {
	return value;
}

template<typename t_value>
constexpr t_value big_to_native_endian(t_value value) {
	return swap_byte_order(value);
}
