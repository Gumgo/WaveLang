...
c_task_data_type::c_task_data_type() {
	m_primitive_type = e_task_primitive_type::k_real;
	m_flags = 0;
}
...
c_task_qualified_data_type::c_task_qualified_data_type() {
	m_qualifier = e_task_qualifier::k_in;
}
...
c_task_qualified_data_type c_task_qualified_data_type::invalid() {
	return c_task_qualified_data_type(c_task_data_type::invalid(), e_task_qualifier::k_count);
}
...
bool c_task_qualified_data_type::is_legal() const {
	...
	if (!m_data_type.get_primitive_type_traits().is_dynamic && m_qualifier != e_task_qualifier::k_in) {
		// Static-only types must only be used as inputs, e.g. a task cannot output a new string
		return false;
	}

	if (m_data_type.is_array() && m_qualifier != e_task_qualifier::k_in) {
