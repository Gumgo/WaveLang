...
void c_voice_allocator::allocate_voices_for_chunk(
	...
	for (uint32 index = 0; index < m_voices.size(); index++) {
	...
	for (size_t event_index = 0; event_index < controller_events.get_count(); event_index++) {
		...
		if (controller_event.controller_event.event_type != k_controller_event_type_note_on) {
			// Skip all events except for note-on
			continue;
		}
		...
		// Grab the first voice in the list. This will first attempt to grab inactive voices, then will grab the oldest
		// active voice.
