...
namespace buffer_iterator_internal {
	...
	template<>
	class c_buffer_iterator<8, e_buffer_iterator_type::k_constant, const c_real_buffer *>
		: public c_constant_buffer_iterator_base {
	public:
		c_buffer_iterator(const c_real_buffer *buffer) : m_constant(buffer->get_constant()) {}
		const real32x8 &get() const { return m_constant; }
	...
	template<>
	class c_buffer_iterator<8, e_buffer_iterator_type::k_input, const c_real_buffer *>
		: public c_input_buffer_iterator_base {
	public:
		c_buffer_iterator(const c_real_buffer *buffer) : m_pointer(buffer->get_data()) {}
		real32x8 get() const { return real32x8(m_pointer); }
		void advance() { m_pointer += 8; }
	...
	template<>
	class c_buffer_iterator<8, e_buffer_iterator_type::k_output, c_real_buffer *> {
	public:
		c_buffer_iterator(c_real_buffer *buffer)
			: m_buffer(buffer)
			, m_pointer(buffer->get_data()) {}
	...
	public:
		...
		int32x4 get() const {
			...
			// Shift the relevant bit into the LSB position, mask out all the other bits, and negate to extend a 1 to
			// all bits
...
#if IS_TRUE(SIMD_256_ENABLED)
	template<>
	class c_buffer_iterator<8, e_buffer_iterator_type::k_constant, const c_bool_buffer *>
		: public c_constant_buffer_iterator_base {
	public:
		c_buffer_iterator(const c_bool_buffer *buffer) : m_constant(-static_cast<int32>(buffer->get_constant())) {}
		const int32x8 &get() const { return m_constant; }
	...
	template<>
	class c_buffer_iterator<8, e_buffer_iterator_type::k_input, const c_bool_buffer *>
		: public c_input_buffer_iterator_base {
	public:
		c_buffer_iterator(const c_bool_buffer *buffer)
			: m_pointer(buffer->get_data())
		...
		int32x8 get() const {
			// Duplicate 8 contiguous bits ABCDEFGH into elements
			// [AA..., BB..., CC..., DD..., EE..., FF..., GG..., HH...]
			int32x8 value(static_cast<int32>(static_cast<uint32>(*m_pointer) >> m_bit) & 255);
			// Shift the relevant bit into the LSB position, mask out all the other bits, and negate to extend a 1 to
			// all bits
			return -((value >> int32x8(0, 1, 2, 3, 4, 5, 6, 7)) & int32x8(1));
		}
	...
	template<>
	class c_buffer_iterator<8, e_buffer_iterator_type::k_output, c_bool_buffer *> {
	public:
		c_buffer_iterator(c_bool_buffer *buffer)
			: m_buffer(buffer)
			, m_pointer(buffer->get_data())
		...
		void advance() {
			// Pack LSB from each element into a contiguous block and shift to the proper offset
			int32 value = ((m_value & int32x8(1)) << int32x8(0, 1, 2, 3, 4, 5, 6, 7)).sum_elements().first_element();
			m_cached_value |= value << m_bit;
			m_bit = (m_bit + 8) & 31;
			if (m_bit == 0) {
				*m_pointer = m_cached_value;
	...
	template<>
	class c_buffer_iterator<256, e_buffer_iterator_type::k_constant, const c_bool_buffer *>
		: public c_constant_buffer_iterator_base {
	public:
		c_buffer_iterator(const c_bool_buffer *buffer) : m_constant(-static_cast<int32>(buffer->get_constant())) {}
		const int32x8 &get() const { return m_constant; }
	...
	template<>
	class c_buffer_iterator<256, e_buffer_iterator_type::k_input, const c_bool_buffer *>
		: public c_input_buffer_iterator_base {
	public:
		c_buffer_iterator(const c_bool_buffer *buffer) : m_pointer(buffer->get_data()) {}
		int32x8 get() const { return int32x8(m_pointer); }
		void advance() { m_pointer += 8; }
	...
	template<>
	class c_buffer_iterator<256, e_buffer_iterator_type::k_output, c_bool_buffer *> {
	public:
		c_buffer_iterator(c_bool_buffer *buffer)
			: m_buffer(buffer)
			, m_pointer(buffer->get_data()) {}
